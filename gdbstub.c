...
/* Set to 1 to enable remote protocol debugging output. This output is similar to the one produced
 * by the gdbserver's --remote-debug flag with some additions. Anyway, the main debug prints are:
 ...
 * Select the filesystem on which vFile operations with filename arguments will operate. This is required for GDB
 * to be able to access files on remote targets where the remote stub does not share a common filesystem with the
 * inferior(s). If pid is nonzero, select the filesystem as seen by process pid. If pid is zero, select the filesystem
 * as seen by the remote stub. Return 0 on success, or -1 if an error occurs. If vFile:setfs: indicates success,
 * the selected filesystem remains selected until the next successful vFile:setfs: operation.
*/
 ...
 *
 * --- From the GDB remote protocol documentation ---
 * Open a file at filename and return a file descriptor for it, or return -1 if an error occurs. The filename is
 * a string, flags is an integer indicating a mask of open flags (see Open Flags), and mode is an integer indicating
 * a mask of mode bits to use if the file is created (see mode_t Values). See open, for details of the open flags
 * and mode values.
 */
static void handle_v_file_open(GArray *params, void *user_ctx)
{
    ...
    // On Linux we call the do_openat syscall on behalf of the inferior as it handles special filepaths properly
    // like the /proc/$pid files, which are fetched by GDB for certain info (such as `info proc mappings`).
#ifdef CONFIG_LINUX
 ...
 *
 * --- From the GDB remote protocol documentation ---
 * Return the full absolute name of the file that was executed to create a process running on the remote system.
 * The annex specifies the numeric process ID of the process to query, encoded as a hexadecimal number. If the annex
 * part is empty the remote stub should return the filename corresponding to the currently executing process.
 *
 * This packet is not probed by default; the remote stub must request it, by supplying an appropriate
 * ‘qSupported’ response (see qSupported).
 */
...
#if defined(CONFIG_USER_ONLY)
    // In QEMU user-mode emulation we want to return the real PID of the proces as this allows us to
    // return proper view of /proc/$pid files as seen by the inferior
    max_pid = getpid() - 1;
#endif
